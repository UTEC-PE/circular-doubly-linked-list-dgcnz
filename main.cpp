...
int main(int argc, char *argv[]) {
  ...
  assert(test.size() == numberOfElements &&
         "Something is wrong with the push methods");
  ...
  assert(test.size() == newNumbersSize &&
         "Something is wrong with the pop functions");
  ...
  assert(test.front() == *ite &&
         "Something is wrong with the front method or the iterator");

  const int position = generateRandomInt(0, numberOfElements - 1);
  for (int i = 0; i < position; i++) {
    ++ite;
  }
  assert(test.get(position) == *ite &&
         "Something is wrong with the get method or the iterator");

  ite = test.begin();
  for (int i = 0; i < numberOfElements - 1; i++) {
    ++ite;
  }
  assert(test.back() == *ite &&
         "Something is wrong with the back method or the iterator");

  --ite;
  assert(test.get(numberOfElements - 2) == *ite &&
         "Something is wrong with the iterator (-- operator)");
